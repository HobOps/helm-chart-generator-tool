...
from tests.core.modules.helm_management.fixture import fixture_filtered_statefulset_helm_values_config_data
...
def test_deployment_helm_config_values_filter(fixture_filtered_deployment_helm_values_config_data, fixture_filtered_statefulset_helm_values_config_data):
    ...
    helm_statefulset_env_vars_filter = HelmStatefulSetEnvVarsFilter(config_data=env_var_pattern)
    helm_values_data_filtered = helm_statefulset_env_vars_filter.process(conf=fixture_filtered_deployment_helm_values_config_data)

    assert helm_values_data_filtered == fixture_filtered_statefulset_helm_values_config_data
