...
def test_path_creator_with_valid_params():
    ...
    fake_file_path = PathFaker(target_path=target_file_path, target_path_type=path_types_values.file, parent_path=fake_folder_path)
...
def test_path_creator_with_not_valid_path_format():
    ...
    fake_file_path = PathFaker(target_path=f"/{target_file_path}", target_path_type=path_types_values.file, parent_path=fake_folder_path)

    expected_error_message = f"Error target_path: home/user1 is not a valid path format"

    with pytest.raises(ValueError) as err:
        path_creator = SimplePathCreator(root_path=invalid_root_path)
